...
class FccSoapClient:
    """
    Singleton class for interacting with the FCC BrueBoxService via SOAP.
    Ensures only one client instance is created to manage connections efficiently.
    """
    ...
    def __new__(cls, wsdl_url):
        ...
        Ensures that only one instance of FccSoapClient is created.
        """
        if cls._instance is None:
            cls._instance = super(FccSoapClient, cls).__new__(cls)
            cls._instance._initialize(wsdl_url) # Initialize the new instance
        return cls._instance

    def _initialize(self, wsdl_url):
        """
        Internal initialization method for the FccSoapClient instance.
        Sets up the WSDL URL, client, session, and transport.
        """
        logger.info(f"Initializing FccSoapClient with WSDL: {wsdl_url}")
        ...
        self._seq_no = 0 # Sequen number for Glory FCC request (auto increment)
        self._seq_lock = threading.Lock() # Lock for thread-safe increment of sequence number (used in FCC requests)
        self._connect_client() # Establish the initial connection to the SOAP service
    ...
    def get_service_instance(self):
        """
        Returns the Zeep service proxy instance (e.g., client.service).
        If the service proxy is not available (e.g., due to initial failure or disconnection),
        it attempts to reconnect.
        """
        if self.service_proxy is None:
            logger.warning("FCC SOAP service proxy not available. Attempting to reconnect....")
            self._connect_client() # Reattempt connection, which will re-create session/transport if needed
    ...
    def change_operation(self, amount, denominations):
        ...
        try:
            ...
            # It's better practice to log the error and then re-raise it
            # so the calling code knows the operation failed.
            raise e
    ...
    # NOTE: This operation is working, I just commented it out to test the new collect methods.
    # def collect(self, session_id: str, scope: str = "all", plan: str = "full", target_float=None) -> dict:
    #     """
    #     Move cash from recyclers to collection container.
    #     - scope: "all" | "notes" | "coins"
    #     - plan : "full" | "target_float"
    #     - target_float: {"notes":[{"value":..,"qty":..}], "coins":[...]}
    #     """
    #     svc = self.get_service_instance()
    #     if svc is None:
    #         raise RuntimeError("FCC SOAP service is not available")
    ...
    #         "SeqNo": "",
    #         "SessionID": str(session_id),
    #         # Minimal safe body; adjust per WSDL.
    #         # Many firmwares accept just an Option/type.
    #         "Option": {"type": 0},  # TODO: verify/type-code for collect mode
    #         # If your WSDL supports keeping a float, translate target_float here.
    #         # e.g. "Cash": {"type": 7, "Denomination": [...]}  # <-- EXAMPLE ONLY
    #     }
    #     logger.info("CollectOperation req: %s", req)
    #     resp = svc.CollectOperation(**req)  # name per WSDL
    #     return serialize_zeep_object(resp)
    ...
    def register_event(self, *, url: str, port: int, destination_type: int = 0,
        ...
        try:
            # <-- exact method name from your ops list
